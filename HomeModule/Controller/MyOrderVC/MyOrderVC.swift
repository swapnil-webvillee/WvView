...
class MyOrderVC: UIViewController , UITableViewDelegate, UITableViewDataSource {
    ...
    {
        var cell = tableView.dequeueReusableCell(withIdentifier: "CustomTCellOrder") as? CustomTCellOrder
        if cell == nil
        {
            let arr = Bundle.main.loadNibNamed("CustomTCellOrder", owner: self, options: nil)
            cell = arr?.first as? CustomTCellOrder
        }
        
       
        cell?.lblStatus.isHidden = indexPath.row%2 == 0
        cell?.btnTrackOrder.isHidden = !(cell?.lblStatus.isHidden)!
